...
import osimport uuidimport shutilimport subprocessimport base64import reimport unicodedataimport urllib.request

from fastapi import FastAPI, HTTPExceptionfrom fastapi.staticfiles import StaticFiles

app = FastAPI()

BASE_DIR = "/tmp/ffmpeg_render"AUDIO_DIR = os.path.join(BASE_DIR, "audio")VIDEO_DIR = os.path.join(BASE_DIR, "video")FONTS_DIR = os.path.join(BASE_DIR, "fonts")IMAGE_DIR = os.path.join(BASE_DIR, "images")CLIPS_DIR = os.path.join(BASE_DIR, "clips")
...
MIN_SCENE_DURATION = 0.5MAX_CLONE_PAD_PER_SCENE = 0.12

HOOK_CARD_START = 0.00HOOK_CARD_END = 0.00

HOOK_WORD_1_START = 0.12HOOK_WORD_2_START = 0.24HOOK_WORD_3_START = 0.42
...
AI_VIDEO_READABILITY_FILTER = "eq=contrast=1.04=1.10"

FPS = 24OUTPUT_WIDTH = 720OUTPUT_HEIGHT = 1280

os.makedirs(AUDIO_DIR, exist_ok=True)os.makedirs(VIDEO_DIR, exist_ok=True)os.makedirs(FONTS_DIR, exist_ok=True)os.makedirs(IMAGE_DIR, exist_ok=True)os.makedirs(CLIPS_DIR, exist_ok=True)

APP_FONTS_DIR = "/app/fonts"

POD TikTok captions use the bundled Bebas Neue font.

APP_FONT_FILE = os.path.join(APP_FONTS_DIR, "BebasNeue-Regular.ttf")RUNTIME_FONT_FILE = os.path.join(FONTS_DIR, "BebasNeue-Regular.ttf")

Use the same font for hook, punch, CTA and captions unless another impact font is added later.

APP_HUD_FONT_CANDIDATES = [os.path.join(APP_FONTS_DIR, "BebasNeue-Regular.ttf"),]RUNTIME_HUD_FONT_FILE = os.path.join(FONTS_DIR, "BebasNeue-Regular.ttf")

if os.path.exists(APP_FONT_FILE) and not os.path.exists(RUNTIME_FONT_FILE):shutil.copy(APP_FONT_FILE, RUNTIME_FONT_FILE)

if not os.path.exists(RUNTIME_HUD_FONT_FILE):for candidate in APP_HUD_FONT_CANDIDATES:if os.path.exists(candidate):shutil.copy(candidate, RUNTIME_HUD_FONT_FILE)break

def get_hud_font_file() -> str:"""Returns the impact font for hook/punch/CTA, with a safe fallback."""if os.path.exists(RUNTIME_HUD_FONT_FILE):return RUNTIME_HUD_FONT_FILEreturn RUNTIME_FONT_FILE

app.mount("/video", StaticFiles(directory=VIDEO_DIR), name="video")

ASS_WHITE = r"\c&HFFFFFF&"  # LBN white, ASS BGRASS_GOLD = r"\c&H5AC1E6&"   # LBN warm gold, ASS BGR (#E6C15A)

IMPACT_WORDS = {"MEMORIA","RECUERDO","RECUERDOS","RENTA","ALQUILER","CIUDAD","SISTEMA","FUTURO","HUMANO","HUMANA","HUMANOS","REAL","SINTÉTICO","SINTETICO","ILEGAL","PROHIBIDO","VENDIÓ","VENDIO","VENDIDA","VENDER","COMPRÓ","COMPRO","BORRÓ","BORRO","PERDIÓ","PERDIO","OLVIDÓ","OLVIDO","AMOR","MADRE","PADRE","FAMILIA","CUERPO","ROSTRO","IDENTIDAD","VIDA","MUERTE","EMOCIÓN","EMOCION","MENTIRA","VERDAD","POBRE","LUJO","PISO","DISTRITO","ARCHIVO","CRÉDITO","CREDITO","NOVA",}

def normalize_token(value: str) -> str:text = str(value or "").upper()text = "".join(c for c in unicodedata.normalize("NFD", text)if unicodedata.category(c) != "Mn")text = re.sub(r"[^A-ZÑ]+", "", text)return text

def escape_ffmpeg_path(path: str) -> str:return (path.replace("\", "\\").replace(":", "\:").replace("'", r"'").replace(",", r",").replace("[", r"[").replace("]", r"]"))

def escape_drawtext_value(value: str) -> str:if not value:return ""return (str(value).replace("\", "\\").replace(":", "\:").replace(",", "\,").replace("'", "’").replace("%", "\%").replace("[", "\[").replace("]", "\]").replace("=", "\=").replace(";", "\;").replace("\n", " ").replace("\r", " "))

def clean_display_text(value: str, max_words: int = 5) -> str:text = str(value or "").strip()text = text.replace("“", "").replace("”", "").replace('"', "")text = re.sub(r"\s+", " ", text)text = text.upper()
...
def split_headline(text: str) -> tuple[str, str]:words = clean_display_text(text, max_words=5).split()
...
def build_hook_impact_lines(text: str) -> list[str]:"""Hook HUD layout: always 3 vertical impact hits.
...
def split_truth_punch_lines(text: str) -> list[str]:words = clean_display_text(text, max_words=4).split()
...
def split_cta_phrase_lines(text: str) -> list[str]:"""Backward-compatible wrapper for legacy callers.
...
def adjust_font_size_for_text(text: str, base_size: int, min_size: int = 72) -> int:plain = str(text or "")plain = plain.replace("\,", ",")plain = plain.replace("\:", ":")plain = re.sub(r"\s+", " ", plain).strip()char_count = len(plain)
...
def adjust_hud_font_size_for_width(text: str,base_size: int,min_size: int = 44,max_width: int = 600,char_width_ratio: float = 0.68,) -> int:"""Conservative font scaler for Archivo Black impact text.
...
def format_reference_stamp(value: str) -> str:"""Converts long world metadata into a short, safe stamp.
...
def clean_cta_display_text(value: str, max_chars: int = 76) -> str:"""Cleans CTA text for the final comment card.
...
def wrap_words_balanced(text: str, max_lines: int = 2, max_chars: int = 18) -> list[str]:text = clean_cta_display_text(text, max_chars=76)words = text.split()
...
def normalize_cta_label(label: str) -> str:label = clean_cta_display_text(label, max_chars=32)label = label.rstrip(":").strip()
...
def split_cta_visual_text_into_lines(text: str, max_lines: int = 4, max_chars: int = 18) -> list[str]:"""Converts a long CTA into 2-4 short visual lines that fit inside the final HUD.
...
def extract_quoted_cta(call_to_action: str, hook: str = "", guion: str = "") -> str:text = str(call_to_action or "")context = f"{call_to_action} {hook} {guion}".lower()
...
def extract_cta_visual_parts(call_to_action: str, hook: str = "", guion: str = "") -> tuple[str, str]:"""Returns a readable CTA label and phrase for logs/backward compatibility.
...
def build_cta_visual_lines(call_to_action: str, hook: str = "", guion: str = "") -> list[str]:text = clean_cta_display_text(call_to_action, max_chars=76)
...
def extract_truth_punch_text(guion: str) -> str:text = str(guion or "").strip().lower()
...
def compute_truth_punch_window(voice_duration: float) -> tuple[float, float] | None:"""Computes the mid-video truth punch window.
...
def get_audio_duration(audio_path: str) -> float:probes = [["ffprobe","-v", "error","-show_entries", "format=duration","-of", "default=noprint_wrappers=1=1",audio_path,],["ffprobe","-v", "error","-show_entries", "stream=duration","-of", "default=noprint_wrappers=1=1",audio_path,],]
...
def download_file(url: str, path: str) -> str:urllib.request.urlretrieve(url, path)return path

def preferred_scene_weights(clip_count: int) -> list[float]:"""Defines the visual pacing preference for ai_video_clean_N.
...
def compute_scene_durations(total_duration: float,clip_count: int,clip_durations: list[float] | None = None) -> list[float]:"""Computes scene durations for the background video.
...
def scene_crop_expression(scene_index: int) -> tuple[str, str]:patterns = [("(iw-ow)/2+16sin(t0.35)", "(ih-oh)/2-10cos(t0.25)"),("(iw-ow)/2-18sin(t0.32)", "(ih-oh)/2+12sin(t0.25)"),("(iw-ow)/2+14sin(t0.30)", "(ih-oh)/2+12cos(t0.24)"),("(iw-ow)/2-16cos(t0.28)", "(ih-oh)/2-12sin(t0.24)"),("(iw-ow)/2+10sin(t0.25)", "(ih-oh)/2+10cos(t0.22)"),]return patterns[scene_index % len(patterns)]

def build_background_from_videos(clip_paths: list,output_path: str,total_duration: float,job_id: str) -> None:n = len(clip_paths)if n == 0:raise RuntimeError("No clip paths received")
...
def build_background_from_images(image_paths: list,output_path: str,total_duration: float,job_id: str) -> None:n = len(image_paths)if n == 0:raise RuntimeError("No image paths received")
...
def build_reference_filter(referencia_biblica: str, start_time: float = REFERENCE_START_TIME) -> str:"""Keeps the existing request field name for Make compatibility, but renders itas a short cinematic location/year stamp for this channel."""if not referencia_biblica or not referencia_biblica.strip():return ""
...
def add_pop_drawtext(filters: list,safe_font_path: str,text: str,final_size: int,fontcolor: str,center_y: int,start_time: float,end_time: float,borderw: int = 6,shadow: int = 3,overshoot_scale: float = 1.10,start_scale: float = 0.82,phase1_duration: float = 0.06,phase2_duration: float = 0.14,):if not text:return
...
def build_hook_card_filters(hook_visual_text: str) -> list:if not os.path.exists(RUNTIME_FONT_FILE):return []
...
def build_truth_punch_filters(guion: str,voice_duration: float,truth_punch_text: str = "") -> list:if not os.path.exists(RUNTIME_FONT_FILE):return []
...
def build_cta_card_filters(call_to_action: str,hook: str,guion: str,cta_start_time: float,final_duration: float) -> list:if not os.path.exists(RUNTIME_FONT_FILE):return []
...
def validate_cta_for_render(call_to_action: str) -> None:text = str(call_to_action or "").strip()
...
def seconds_to_ass_time(seconds: float) -> str:hours = int(seconds // 3600)minutes = int((seconds % 3600) // 60)secs = seconds % 60return f"{hours}:{minutes:02d}:{secs:05.2f}"

def escape_ass_text(text: str) -> str:return (str(text).replace("\", r"\").replace("{", r"{").replace("}", r"}"))

def speed_up_alignment(alignment: dict, speed: float) -> dict:return {"characters": alignment.get("characters", []),"character_start_times_seconds": [float(x) / speed for x in alignment.get("character_start_times_seconds", [])],"character_end_times_seconds": [float(x) / speed for x in alignment.get("character_end_times_seconds", [])],}

def build_words_from_alignment(alignment: dict) -> list:characters = alignment.get("characters", [])starts = alignment.get("character_start_times_seconds", [])ends = alignment.get("character_end_times_seconds", [])
...
def tokenize_for_alignment_match(text: str) -> list[str]:"""Converts text into normalized word tokens for matching against ElevenLabs alignment.Works with any CTA wording: Comenta, Sígueme, Escribe, Guarda, Ora, etc."""raw = str(text or "").strip()
...
def find_sequence_start_in_words(words: list,target_text: str,fallback_time: float,min_match_tokens: int = 4,search_after_ratio: float = 0.45) -> float:"""Finds the start time of target_text inside ElevenLabs word alignment.
...
def split_word_items_two_lines(word_items: list, max_line_chars: int = 18) -> list:if not word_items:return []
...
def group_words_into_cues(words: list, max_words: int = 4, max_chars: int = 26) -> list:cues = []bucket = []
...
def build_line_groups(word_items: list, max_line_chars: int = 16) -> list:split_lines = split_word_items_two_lines(word_items, max_line_chars=max_line_chars)groups = []flat_index = 0
...
def should_highlight_word(word: str) -> bool:normalized = normalize_token(word)if not normalized:return False

normalized_impact = {normalize_token(x) for x in IMPACT_WORDS}
return normalized in normalized_impact

def build_ass_dialogue_text(groups: list, active_index: int | None = None) -> str:line_texts = []
...
def write_ass_subtitles(subtitles_path: str,cues: list,cta_start_time: float | None = None,truth_punch_window: tuple[float, float] | None = None,):header = """[Script Info]ScriptType: v4.00+PlayResX: 720PlayResY: 1280WrapStyle: 2ScaledBorderAndShadow: yes

[V4+ Styles]Format: Name, Fontname, Fontsize, PrimaryColour, SecondaryColour, OutlineColour, BackColour, Bold, Italic, Underline, StrikeOut, ScaleX, ScaleY, Spacing, Angle, BorderStyle, Outline, Shadow, Alignment, MarginL, MarginR, MarginV, EncodingStyle: Default,Bebas Neue,76,&H00FFFFFF,&H00FFFFFF,&H00000000,&H64000000,-1,0,0,0,100,100,0,0,1,4,0,2,80,80,285,1
...
def validate_video_filter_for_ffmpeg(filter_value: str) -> None:"""Guardrail against FFmpeg drawtext parser failures.
...
@app.get("/")def health():return {"status": "running","font_exists": os.path.exists(RUNTIME_FONT_FILE),"font_path": RUNTIME_FONT_FILE,"hud_font_exists": os.path.exists(RUNTIME_HUD_FONT_FILE),"hud_font_path": get_hud_font_file(),"music_exists": os.path.exists(MUSIC_FILE),"music_path": MUSIC_FILE,"end_tail_duration": END_TAIL_DURATION,"scene_duration_mode": "pod_single_avatar_video_no_tail","max_clone_pad_per_scene": MAX_CLONE_PAD_PER_SCENE,"hook_card_start": HOOK_CARD_START,"hook_card_end": HOOK_CARD_END,"hook_word_1_start": HOOK_WORD_1_START,"hook_word_2_start": HOOK_WORD_2_START,"hook_word_3_start": HOOK_WORD_3_START,"hook_card_mode": "disabled_no_cards","truth_punch_mode": "disabled_for_pod","cta_card_mode": "disabled_no_cards","cta_detection_mode": "disabled_no_visual_cta","reference_start_time": REFERENCE_START_TIME,"cta_card_duration": CTA_CARD_DURATION,"truth_punch_duration": TRUTH_PUNCH_DURATION,"music_required": False,"cta_card_required": False,"voice_starts_at": "0.00s","sfx_enabled": False,"render_style": "pod_tiktok_avatar_captioned",}

class RenderRequest(BaseModel):numero_regla: str = ""hook: str = ""hook_visual_text: str = ""call_to_action: str = ""guion: strtruth_punch_text: str = ""subtitles_mode: str = "dynamic"audio_base64: strnormalized_alignment: dict
...
@app.post("/render")async def render_video(data: RenderRequest):if not os.path.exists(RUNTIME_FONT_FILE):raise HTTPException(status_code=500,detail=f"La fuente no existe en runtime: {RUNTIME_FONT_FILE}")
